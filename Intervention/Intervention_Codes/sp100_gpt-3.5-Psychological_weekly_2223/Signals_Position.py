...
As a ordinary shareholders, you start with an initial equity of $1,000,000. Your task is to make trading decisions based on the provided historical dataset to maximize your equity. \

The dataset includes the following columns: ["date", "code", "open", "high", "low", "close", "change", "decision", "hold_num", "equity"]. \
...
The "hold_num" column records the number of shares you hold after trading actions.\
...
Please be aware that when you sell stocks, you need to pay a handling fee of (0.0021% of the transaction amount + US$0.000119 per share). \
...
def signal_and_position(input_data, pos_div=0.2):
    ...
    # Add "reference" to mark the starting position for buying, used to adjust GPT’s psychological reference point, soft adjustment, starts buying from the midpoint
    # reference = -1
    ...
    for i in range(11, len(input_data)):
        ...
        for k in range(11, i):
            ...
            if counter.at[k-1, 'decision'] == "buy":
                # Prevent division by zero, i.e., ensure affordability even if the strategy is "buy"
        ...
        # Process each row of data here, e.g., printing or performing other operations
        user_prompt = f"{row_c}"
        if counter.at[i-1, 'hold_num'] == 0:
            user_prompt += "\n" + "Please note that you have not bought any shares yet."
        else:

            user_prompt += "\n" + f'Please note that the average purchase price of your current holdings is {buy_price}, while the current price is {input_data.at[i-1, "close"]}.'

            user_prompt += "\n" + "It's important to inform you that, when the current price of a stock is higher than the purchase price, \
            selling the stock amidst recent consecutive increases may result in potential loss of gains. Conversely, \
            when the current price of a stock is lower than the purchase price, continuing to hold the stock during recent consecutive decreases \
            may lead to even greater losses."
            
            # user_prompt += " When the open price of the stock has been generally rising over the past 7 periods, \
            #     buying the stock may lead to good returns."
